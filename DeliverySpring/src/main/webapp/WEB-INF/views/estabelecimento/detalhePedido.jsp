...
<body>
	...
	<div id="conteudo" style="width: 1000px; margin: 0 auto;">
		...
		<div style="margin-top: 60px;">
			<c:if test="${pedido.status == 'SOLICITADO' }">
				<a class="btn btn-primary" href="<c:url value = "/estabelecimento/atenderPedido/${pedido.id}"></c:url>">Atender pedido</a>
			</c:if>
			<c:if test="${pedido.status == 'ATENDIDO' }">
				<a class="btn btn-primary" href="<c:url value = "/estabelecimento/finalizarPedido/${pedido.id}"></c:url>">Finalizar pedido</a>
			</c:if>
		</div>
